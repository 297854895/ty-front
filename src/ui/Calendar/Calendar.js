...
export default class Calendar extends React.Component {
  ...
  toggleYear = type => () => this.setState({ currentYear: this.state.currentYear + 1 * (type === 'next' ? 1 : -1) }, () => {
    const { toggleYearMonth } = this.props
    if (!toggleYearMonth || typeof toggleYearMonth !== 'function') return
    toggleYearMonth(this.state.currentMonth, this.state.currentYear)
  })
  toggleMonth = month => {
    const toggleYearMonth = this.props.toggleYearMonth
    this.setState({ currentMonth: month }, () => {
      if (!toggleYearMonth || typeof toggleYearMonth !== 'function') return
      toggleYearMonth(month, this.state.currentYear)
    })
  }
  toggleDay = day => {
    if (day.high) return
...
Calendar.propTypes = {
  ...
  toggleYearMonth: PropTypes.func,
